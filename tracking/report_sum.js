Ext.require(["*", "Ext.grid.plugin.CellEditing", "Ext.grid.*", "Ext.data.*", "Ext.util.*", "Ext.toolbar.Paging"]);
var model_detail_sum = Ext.define("detail_kapal_sum", {
        extend: "Ext.data.Model",
        fields: [{
            name: "date",
            type: "date"
        }, "working distance", "average speed", "working hours ME1", "working hours ME2", "working hours AE1", "working hours AE2", "ME1 daily consumption", "ME2 daily consumtion", "ME1 fuel rate", "ME2 fuel rate", "AE1 fuel rate", "AE2 fuel rate", "ME1 average rpm", "ME2 average rpm", "AE1 average rpm", "AE2 average rpm", "AE1 consumtion", "AE2 consumtion", "vessel", "Remaining on board", "Last fuel loading", "Total Consumption ME", "Total Consumption AE", "Total Consumption", "Total Working Hours ME", "Total Working Hours AE", "Total Working Hours", "Total Working Hours"]
    }),
    tgl_sel_sum = "",
    dtt = JSON.parse(atob(Ext.util.Cookies.get("marine"))),
    store_detail_sum = Ext.create("Ext.data.Store", {
        model: model_detail_sum,
        autoLoad: !0,
        proxy: {
            type: "ajax",
            url: getAPI() + "/get_data_summary_ship",
            method: "GET",
            timeout: 6e4
        }
    }),
    tabel_r_sum = Ext.create("Ext.grid.Panel", {
        id: "table_ship_sum",
        title: "Summary",
        store: store_detail_sum,
        flex: 4,
        columns: [{
            header: "Vessel",
            align: "center",
            width: 100,
            dataIndex: "vessel",
            locked: !0
        }, {
            header: "",
            columns: [{
                header: "Working Distance",
                align: "center",
                columns: [{
                    header: "Km",
                    align: "center",
                    width: 100,
                    dataIndex: "working distance",
                    renderer: function (v) {
                        return parseFloat(v).toFixed(2)
                    }
                }]
            }]
        }, {
            header: "",
            columns: [{
                header: "Average Speed",
                align: "center",
                columns: [{
                    header: "Km/h",
                    align: "center",
                    width: 100,
                    dataIndex: "average speed",
                    renderer: function (v) {
                        return parseFloat(v).toFixed(2)
                    }
                }]
            }]
        }, {
            header: "",
            columns: [{
                header: "Remaining on Board",
                align: "center",
                columns: [{
                    header: "Liter",
                    align: "center",
                    width: 150,
                    dataIndex: "Remaining on board",
                    renderer: function (v) {
                        return parseFloat(v).toFixed(2)
                    }
                }]
            }]
        }, {
            header: "",
            columns: [{
                header: "Total Consumption",
                align: "center",
                columns: [{
                    header: "Liter",
                    align: "center",
                    width: 150,
                    dataIndex: "Total Consumption",
                    renderer: function (v) {
                        return parseFloat(v).toFixed(2)
                    }
                }]
            }]
        }, {
            header: "",
            columns: [{
                header: "Last Fuel Loding",
                align: "center",
                columns: [{
                    header: "Liter",
                    align: "center",
                    width: 150,
                    dataIndex: "Last fuel loading",
                    renderer: function (v) {
                        return parseFloat(v).toFixed(2)
                    }
                }]
            }]
        }, {
            header: "",
            columns: [{
                header: "Total Engine Hours",
                align: "center",
                columns: [{
                    header: "Hr",
                    align: "center",
                    width: 150,
                    dataIndex: "Total Working Hours",
                    renderer: function (v) {
                        return parseFloat(v).toFixed(2)
                    }
                }]
            }]
        }, {
            header: "Total Main Engine",
            align: "center",
            columns: [{
                header: "Fuel Consumption",
                align: "center",
                columns: [{
                    header: "Liter",
                    align: "center",
                    width: 100,
                    dataIndex: "Total Consumption ME",
                    renderer: function (v) {
                        return parseFloat(v).toFixed(2)
                    }
                }]
            }, {
                header: "Engine Hours",
                align: "center",
                columns: [{
                    header: "Hr",
                    align: "center",
                    width: 100,
                    dataIndex: "Total Working Hours ME",
                    renderer: function (v) {
                        return parseFloat(v).toFixed(2)
                    }
                }]
            }]
        }, {
            header: "PortSide Engine",
            align: "center",
            columns: [{
                header: "Fuel Consumption",
                align: "center",
                columns: [{
                    header: "Liter",
                    align: "center",
                    width: 100,
                    dataIndex: "ME1 daily consumption",
                    renderer: function (v) {
                        return parseFloat(v).toFixed(2)
                    }
                }]
            }, {
                header: "Engine Hours",
                align: "center",
                columns: [{
                    header: "Hr",
                    align: "center",
                    width: 100,
                    dataIndex: "working hours ME1",
                    renderer: function (v) {
                        return parseFloat(v).toFixed(2)
                    }
                }]
            }, {
                header: "Fuel Rate",
                align: "center",
                columns: [{
                    header: "lt/hr",
                    align: "center",
                    width: 100,
                    dataIndex: "ME1 fuel rate",
                    renderer: function (v) {
                        return parseFloat(v).toFixed(2)
                    }
                }]
            }]
        }, {
            header: "StarBoard Engine",
            align: "center",
            columns: [{
                header: "Fuel Consumption",
                align: "center",
                columns: [{
                    header: "Liter",
                    align: "center",
                    width: 100,
                    dataIndex: "ME2 daily consumtion",
                    renderer: function (v) {
                        return parseFloat(v).toFixed(2)
                    }
                }]
            }, {
                header: "Engine Hours",
                align: "center",
                columns: [{
                    header: "Hr",
                    align: "center",
                    width: 100,
                    dataIndex: "working hours ME2",
                    renderer: function (v) {
                        return parseFloat(v).toFixed(2)
                    }
                }]
            }, {
                header: "Fuel Rate",
                align: "center",
                columns: [{
                    header: "lt/hr",
                    align: "center",
                    width: 100,
                    dataIndex: "ME2 fuel rate",
                    renderer: function (v) {
                        return parseFloat(v).toFixed(2)
                    }
                }]
            }]
        }, {
            header: "Total Auxiliary Engine",
            align: "center",
            columns: [{
                header: "Fuel Consumption",
                align: "center",
                columns: [{
                    header: "Liter",
                    align: "center",
                    width: 100,
                    dataIndex: "Total Consumption AE",
                    renderer: function (v) {
                        return parseFloat(v).toFixed(2)
                    }
                }]
            }, {
                header: "Engine Hours",
                align: "center",
                columns: [{
                    header: "Hr",
                    align: "center",
                    width: 100,
                    dataIndex: "Total Working Hours AE",
                    renderer: function (v) {
                        return parseFloat(v).toFixed(2)
                    }
                }]
            }]
        }, {
            header: "PortSide GenSet",
            align: "center",
            columns: [{
                header: "Fuel Consumption",
                align: "center",
                columns: [{
                    header: "Liter",
                    align: "center",
                    width: 100,
                    dataIndex: "AE1 consumtion",
                    renderer: function (v) {
                        return parseFloat(v).toFixed(2)
                    }
                }]
            }, {
                header: "Engine Hours",
                align: "center",
                columns: [{
                    header: "Hr",
                    align: "center",
                    width: 100,
                    dataIndex: "working hours AE1",
                    renderer: function (v) {
                        return parseFloat(v).toFixed(2)
                    }
                }]
            }, {
                header: "Fuel Rate",
                align: "center",
                columns: [{
                    header: "lt/hr",
                    align: "center",
                    width: 100,
                    dataIndex: "AE1 fuel rate",
                    renderer: function (v) {
                        return parseFloat(v).toFixed(2)
                    }
                }]
            }]
        }, {
            header: "StarBoard GenSet",
            align: "center",
            columns: [{
                header: "Fuel Consumption",
                align: "center",
                columns: [{
                    header: "Liter",
                    align: "center",
                    width: 100,
                    dataIndex: "AE2 consumtion",
                    renderer: function (v) {
                        return parseFloat(v).toFixed(2)
                    }
                }]
            }, {
                header: "Engine Hours",
                align: "center",
                columns: [{
                    header: "Hr",
                    align: "center",
                    width: 100,
                    dataIndex: "working hours AE2",
                    renderer: function (v) {
                        return parseFloat(v).toFixed(2)
                    }
                }]
            }, {
                header: "Fuel Rate",
                align: "center",
                columns: [{
                    header: "lt/hr",
                    align: "center",
                    width: 100,
                    dataIndex: "AE2 fuel rate",
                    renderer: function (v) {
                        return parseFloat(v).toFixed(2)
                    }
                }]
            }]
        }]
    }),
    panel_r_sum = {
        dockedItems: [{
            padding: "0 0 0 10",
            xtype: "toolbar",
            dock: "top",
            height: 40,
            items: [{
                padding: "0 0 0 5",
                fieldLabel: "Date ",
                id: "date_total_sum",
                labelWidth: 40,
                editable: !1,
                xtype: "datefield",
                value: new Date,
                maxValue: new Date,
                format: "d-M-Y",
                listeners: {
                    change: function () {
                        tgl_sel_sum = Ext.Date.format(this.getValue(), "Y-m-d")
                    },
                    afterrender: function () {
                        tgl_sel_sum = Ext.Date.format(this.getValue(), "Y-m-d");
                        var param = {
                            user_id: dtt.idu,
                            tgl: tgl_sel_sum,
                            tz: getTimeZone(),
                            type: "data_adhoc"
                        };
                        store_detail_sum.load({
                            params: param
                        })
                    }
                }
            }, {
                padding: "0 0 0 5",
                fieldLabel: "To ",
                id: "date_total_sum2",
                labelWidth: 40,
                editable: !1,
                xtype: "datefield",
                value: new Date,
                maxValue: new Date,
                format: "d-M-Y",
                listeners: {
                    change: function () {
                        tgl_sel_sum2 = Ext.Date.format(this.getValue(), "Y-m-d")
                    },
                    afterrender: function () {
                        tgl_sel_sum2 = Ext.Date.format(this.getValue(), "Y-m-d")
                    }
                }
            }, {
                xtype: "button",
                text: "Submit",
                handler: function () {
                    tgl_sel_sum = Ext.Date.format(Ext.getCmp("date_total_sum").getValue(), "Y-m-d"), tgl_sel_sum2 = Ext.Date.format(Ext.getCmp("date_total_sum2").getValue(), "Y-m-d");
                    var param = {
                        user_id: dtt.idu,
                        tgl: tgl_sel_sum,
                        tgl2: tgl_sel_sum2,
                        tz: getTimeZone(),
                        type: "data_adhoc"
                    };
                    store_detail_sum.load({
                        params: param
                    })
                }
            }, "->", {
                xtype: "button",
                text: "Export to Excel",
                handler: function () {
                    window.open(getAPI() + "/get_data_summary_ship?&user_id=" + dtt.idu + "&tgl=" + tgl_sel_sum + "&tgl2=" + tgl_sel_sum2 + "&tz=" + getTimeZone() + "&type=data_adhoc&export=true&filename=Pelindo3_Summary_" + tgl_sel_sum + "_until_" + tgl_sel_sum2 + ".xlsx")
                }
            }]
        }],
        layout: {
            type: "hbox",
            pack: "start",
            align: "stretch"
        },
        items: [tabel_r_sum]
    };

function getHour(value) {
    if (null == value) return "0 hr";
    if (value <= 0) return "0 hr";
    var hours = Math.floor(value / 60),
        minutes = value % 60,
        hour, min;
    return (hours > 1 ? hours + " hrs " : hours + " hr ") + (minutes > 0 ? parseFloat(minutes).toFixed(0) + " mins" : "")
}